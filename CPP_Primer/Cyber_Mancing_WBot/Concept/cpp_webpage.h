...
// CONSTUCTORS //
    Cpp_webpage();
    Cpp_webpage(std::string title, std::string header, unsigned lev, std::fstream &fpara = fnull, std::string spara = "");

// MEMBER FUNCTIONS //
    std::string title(std::string label);
    std::string paragraph(std::fstream &para = fnull, std::string str = "");
