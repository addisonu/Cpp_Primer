...
// OTHER MEMBER FUNCITONS //
    ...
    {
// Create reference to Word in index to change or empty word to return if its not found
...
// POSTCONDITIONS : If Word objective with word equal to str is in index, a reference to the Word will be returned
    ...
    {
// Empty Word to return if str not found in index
        Word non;
...
// If str not found, return empty Word object
        return non;
    }
...
// Check if word was found by testing if tmp_word is empty Word, if found add object into lhs Word
        if(!tmp_word.get_word().empty()){
            tmp_word += obj;
        }
// If not found, check if word is stop word, if false, add word into index
        else{
